page 50006 "Sales Report-Unposted Entries"
{
  PageType = List;
  ApplicationArea = All;
  ...
  trigger OnAfterGetRecord()begin
    ...
    if GLSetup.Get()then begin
      ...
      if LCY.get(GLSetup."LCY Code")then begin
        ...
        begin //get closes exchange rate
          LCY_CER.Reset();
      ...
      if GLSetup."Additional Reporting Currency" <> '' then begin
        ...
        if AddRepCurr.Get(GLSetup."Additional Reporting Currency")then begin
          ...
          begin //get closest Exchange rate
            ARC_CER.Reset();
    ...
    CustSite.Reset();
    clear(SiteName);
    if CustSite.get(SH."Sell-to Customer No.", SH.Site)then SiteName:=CustSite."Site Name"
    else
      SiteName:='';
  end;
}
