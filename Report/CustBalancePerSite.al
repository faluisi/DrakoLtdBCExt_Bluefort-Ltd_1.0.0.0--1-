report 50001 CustBalancePerSite
{
  DefaultLayout = RDLC;
  ...
  trigger OnAfterGetRecord()var DimSetEntry: Record "Dimension Set Entry";
  ...
  begin
    ...
    DimSetEntry.SetFilter("Dimension Code", FASetup."Site Dimension");
    if not DimSetEntry.FindFirst()then CurrReport.Skip()
    else
    begin
  ...
  trigger OnAfterGetRecord()begin
    ...
    if Format(Cust) <> Format(prevCust)then showDetails:=true
    else
      showDetails:=false;
  end;
  }
