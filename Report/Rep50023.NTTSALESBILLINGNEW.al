report 50023 "NTT SALES BILLING NEW"
{
    DefaultLayout = RDLC;
    RDLCLayout = './RDLC/R50023 NTT Billing Statement NEW.rdl';
    Caption = 'Billing Statement D2R';
    ...
    dataset
    {
        ...
        "Sales Invoice Header")
        {
            ...
            column(ExternalNo_SalesInvHdr;
            ConvertStr("External Document No.", '.', ' '))
            {
            }
            ...
            "Integer")
            {
                ...
                "Integer")
                {
                    ...
                    column(amountnotext; AmountoText[1])
                    {
                    ...
                    "Integer")
                    {
                        ...
                        begin
                            ...
                            FormatNoText(AmountoText, "Sales Invoice Header".Amount, "Sales Invoice Header"."Currency Code");
                            AmountoText[1] := Text025;
                            AmountoText[2] := AmountoText[1];
                    ...
                    "Sales Invoice Line")
                    {
                        ...
                        column(TotalSubTotalC1; TotalSubTotalC1)
                        {
                            AutoFormatExpression = 'USD';
                            AutoFormatType = 1;
                        }
                        column(TotalInvDiscountAmountC1; TotalInvDiscountAmountC1)
                        {
                            AutoFormatExpression = 'USD';
                            AutoFormatType = 1;
                        }
                        column(TotalAmountVATC1; TotalAmountVATC1)
                        ...
                        column(Period_Start;
                        Format("Period Start", 0, '<Month Text,3> <Day>, <Year4>'))
                        {
                        }
                        column(Period_End;
                        Format("Period End", 0, '<Month Text,3> <Day>, <Year4>'))
                        {
                        }
                        //BFT-001 -- end
                        ...
                        trigger OnAfterGetRecord()
                        var
                            taxline: Record "Sales Invoice Line";
                        ...
                        begin
                            ...
                            //calculation for line of leasing - franchise tax
                            //if ("Sales Invoice Line"."No." = 'S0008') or ("Sales Invoice Line"."No." = 'S0009')
                            //  then
                            //     CurrReport.Skip();
                            linedescription := '';
                            linesnetamount := 0;
                            if ("Sales Invoice Line"."No." = 'S0001') or (("Sales Invoice Line"."No." = 'S0002')) then begin

                                linedescription := 'NTT Fee after franchise tax deduction of PHP ';
                                linesnetamount := "Sales Invoice Line".Amount;
                                taxline.SetRange("Document No.", "Sales Invoice Line"."Document No.");
                                taxline.SetRange(Type, taxline.Type::Item);
                                // taxline.SetRange("Line No.", "Sales Invoice Line"."Line No." + 1);
                                taxline.SetFilter("No.", 'S0008|S0009');
                                if (taxline.FindFirst()) then begin
                                    linedescription := linedescription + format(taxline.Amount);
                                    linesnetamount := "Sales Invoice Line".Amount + taxline.Amount;
    ...
    var
        ...
        FormatAddrCountry: Codeunit "Format Addr";
        HasVAT: Boolean;
        ...
        AmountoText: array[2] of Text[80];
    ...
    begin
        if (SalesInvoiceHeader.Site <> '') then begin
            Site.SetFilter(Site."Site Code", SalesInvoiceHeader.Site);
    ...
    begin
        if (Cust."Payment Bank Code" <> '') then begin
            ...
            if (PaymentBank.FindFirst()) then begin
                ...
                BankDetails[6] := PaymentBank.Currency;
                BankDetails[7] := PaymentBank."Bank Address";
                BankDetails[8] := PaymentBank."Bank Address 2";
                HasBank := true;
