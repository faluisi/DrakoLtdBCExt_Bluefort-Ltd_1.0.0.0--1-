report 50005 "Bank Trial Balance"
{
  ...
  //Bank account
  trigger OnAfterGetRecord()begin //1
    //create start date
    Clear(StartDateYear);
    Clear(BLE_Op1BalLCY);
    Clear(BLE_Op1BalFCY);
    ...
    if CloseBalFCY >= 0 then CloseBalFCY_DR:=CloseBalFCY
    else
    ...
    if "Bank Account"."Currency Code" = GLS."LCY Code" then showFCY:=false
    else
      showFCY:=true;
 ...
 'USD': begin
  ...
  showEUR: Boolean;
  showUSD: Boolean;
  procedure CurrencyInArray(CurrCode: Code[10]): Integer begin
