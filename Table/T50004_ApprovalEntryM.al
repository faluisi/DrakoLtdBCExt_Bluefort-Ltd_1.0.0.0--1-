/* table 50004 ApprovalEntryM
{
    ...
    keys
    {
        key(Key1; Company, "Entry No.")
        {
            Clustered = true;
        }
        key(Key2; Company, "Record ID to Approve", "Workflow Step Instance ID", "Sequence No.")
        {
        }
        key(Key3; Company, "Table ID", "Document Type", "Document No.", "Sequence No.", "Record ID to Approve")
        {
        }
        key(Key4; Company, "Approver ID", Status, "Due Date", "Date-Time Sent for Approval")
        {
        }
        key(Key5; Company, "Sender ID")
        {
        }
        key(Key6; Company, "Due Date")
        {
        }
        key(Key7; Company, "Table ID", "Record ID to Approve", Status, "Workflow Step Instance ID", "Sequence No.")
        {
        }
        key(Key8; Company, "Table ID", "Document Type", "Document No.", "Date-Time Sent for Approval")
        {
        }
    }
    ...
    begin
        NotificationEntry.ChangeCompany(rec.Company);
    ...
    begin
        WorkflowEventQueue.ChangeCompany(rec.Company);
    ...
    begin
        ...
        case "Table ID" of
            ...
            DATABASE::Customer:
                begin
                    Customer.ChangeCompany(rec.Company);
    ...
    var
        ...
        NewValue: Text;
    begin
        WorkflowRecordChange.ChangeCompany(rec.Company);
    ...
    begin
        UserSetup.ChangeCompany(rec.Company);
    ...
    local procedure OnAfterGetRecordToApprove(ApprovalEntry: Record ApprovalEntryM; var RecRef: RecordRef; var Result: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterGetCustVendorDetails(var ApprovalEntry: Record ApprovalEntryM; var CustVendorNo: Code[20]; var CustVendorName: Text[100])
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeMarkAllWhereUserisApproverOrSender(var ApprovalEntry: Record ApprovalEntryM; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeShowRecord(var ApprovalEntry: Record ApprovalEntryM; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRecordDetails(var ApprovalEntry: Record ApprovalEntryM; var Details: Text; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRecordCaption(var ApprovalEntry: Record ApprovalEntryM; var Result: Text; var IsHandled: Boolean)
    begin
    end;
}
