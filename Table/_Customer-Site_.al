table 50001 "Customer-Site"
{
  ...
  var FADimMgt: Codeunit FixedAssetDimMgt;
  ...
  trigger OnDelete()var DimensionSetEntry: Record "Dimension Set Entry";
  ...
  procedure CheckUniqueSite(SiteCode: Code[20])var CustSite: Record "Customer-Site";
  begin
    CustSite.Reset();
    CustSite.SetFilter(CustSite."Site Code", "Site Code");
    if CustSite.FindFirst()then Error(Text001, CustSite."Customer No.");
  end;
  procedure UpdateCustOpSite(CustSite: Record "Customer-Site")var COS: Record "Cust-Op-Site";
